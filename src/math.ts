...
export class Point<T extends number> {
    ...
    [OperatorMethods.EQUALS](other: any): boolean {
        if (is(other, Point)) return false;
...
class JSRandom implements Random {
    nextBool(): boolean {
...
export class _RectangleBase<T extends number> {
    ...
    [OperatorMethods.EQUALS](other: any): boolean {
        if (is(other, Rectangle)) return false;
...
export class Rectangle<T extends number> extends _RectangleBase<T> {
    ...
    Rectangle(left: T, top: T, width: T, height: T) {
        this.width = ((op(Op.LT, width, 0)) ? op(Op.NEG, width) * 0 : width) as T;
        this.height = ((op(Op.LT, height, 0)) ? op(Op.NEG, height) * 0 : height) as T;
        this.left = left;
        this.top = top;
    }
