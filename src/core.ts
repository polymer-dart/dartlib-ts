...
class DartPrimitives {
    ...
    static parseInt(
        source: string, radix: int, handleError: (source: string) => int): int {
        checkString(source);
        let re: RegExp = /^\s*[+-]?((0x[a-f0-9]+)|(\d+)|([a-z0-9]+))\s*$/i /*JS('', r'/^\s*[+-]?((0x[a-f0-9]+)|(\d+)|([a-z0-9]+))\s*$/i')*/;
        ...
        if (radix < 10 || decimalMatch == null) {
            ...
            let digitsPart: string = match[digitsIndex] /* JS('String', '#[#]', match, digitsIndex)*/;
    ...
    /** [: r"$".codeUnitAt(0) :] */
    static DOLLAR_CHAR_VALUE: int = 36;

    static extractStackTrace(error: DartError): DartStackTrace {
        return getTraceFromException((error as any).$thrownJsError || error);
    }
...
const _hasErrorStackProperty: boolean = new Error().stack != void 0;
...
class DartStackTrace {
    ...
    static get current(): DartStackTrace {
        ...
        try {
            ...
            return new DartStackTrace.fromError(_);
        }
    }
...
export class _StringStackTrace extends DartStackTrace {
    _stackTrace: string;
...
@DartClass
@Implements(DartStackTrace)
export class _StackTrace extends DartStackTrace {
    _exception;
    _trace: string;
    ...
    @defaultConstructor
    _StackTrace(_exception: any) {
        this._exception = _exception;
    }

    toString(): string {
        if (this._trace != null) return this._trace/* JS('String', '#', _trace) */;
        let trace: string;
...
export var getTraceFromException: (exception: any) => DartStackTrace = (exception: any): DartStackTrace => {
    if (is(exception, DartExceptionAndStackTrace)) {
        return exception.stackTrace;
    }
    if (op(Op.EQUALS, exception, null) || typeof exception != 'object') return new _StackTrace(exception);
    let trace: _StackTrace = exception.$cachedTrace/* JS('_StackTrace|Null', r'#.$cachedTrace', exception) */;
...
class JSString extends String implements DartString, JSIndexable<string> {
    ...
    compareTo(other: DartString | string): int {
 ...
 */
@DartClass
class DartNumber implements DartComparable<num> {
    @defaultFactory
    ...
    static parse(input: string, onError?: (input: string) => num): num {
        ...
        let result = DartInt.parse(source, {onError: DartNumber._returnIntNull});
        if (result != null) return result;
        result = DartDouble.parse(source, DartNumber._returnDoubleNull);
        if (result != null) return result;
...
class JSNumber extends Number implements DartNumber, DartComparable<num> {
    ...
    compareTo(b: num): int {
        if (isNot(b, 'num')) throw argumentErrorValue(b);
        if (this.valueOf() < b) {
            return -1;
            ...
            if (this.valueOf() == 0) {
                let bIsNegative: bool = new DartNumber(b).isNegative;
    ...
    static _MIN_INT32 = -0x80000000;
    static _MAX_INT32 = 0x7FFFFFFF;

    toInt(): int {
        if (this.valueOf() >= JSNumber._MIN_INT32 && this.valueOf() <= JSNumber._MAX_INT32) {
    ...
    ceil(): int {
        if (this.valueOf() >= 0) {
            if (this.valueOf() <= JSNumber._MAX_INT32) {
                let truncated: int = this.valueOf() | 0/* JS('int', '# | 0', this)*/; // converts -0.0 to 0.
                return this.valueOf() == truncated ? truncated : truncated + 1;
            }
        } else {
            if (this.valueOf() >= JSNumber._MIN_INT32) {
                return this.valueOf() | 0 /*JS('int', '# | 0', this)*/;
            }
        }
    ...
    floor(): int {
        if (this.valueOf() >= 0) {
            ...
            if (this.valueOf() >= JSNumber._MIN_INT32) {
                let truncated: int = this.valueOf() | 0 /*JS('int', '# | 0', this)*/;
                return this.valueOf() == truncated ? truncated : truncated - 1;
            }
        }
    ...
    clamp(lowerLimit: num, upperLimit: num): num {
        if (isNot(lowerLimit, 'num')) throw argumentErrorValue(lowerLimit);
        if (isNot(upperLimit, 'num')) throw argumentErrorValue(upperLimit);
    ...
    toStringAsFixed(fractionDigits: int): string {
        ...
        if (this.valueOf() == 0 && this.isNegative) return `-${result}`;
        return result;
    }
    ...
    _tdivSlow(other: num): int {
        ...
        if (quotient >= JSNumber._MIN_INT32 && quotient <= JSNumber._MAX_INT32) {
    ...
    lshift(other: num): num {
        if (isNot(other, 'num')) throw argumentErrorValue(other);
        if (other /*JS('num', '#', other)*/ < 0) throw argumentErrorValue(other);
    ...
    rshift(other: num): num {
        if (false) this._shrReceiverPositive(other);
        if (isNot(other, 'num')) throw argumentErrorValue(other);
        if (other /*JS('num', '#', other)*/ < 0) throw argumentErrorValue(other);
 ...
 */
@DartClass
class DartInt extends DartNumber {
    @defaultFactory
    ...
    //ignore: const_factory
    @namedFactory
    protected static _fromEnvironment(name: string, _?: { defaultValue: int }): DartInt {
        throw "external";
    }

    static fromEnvironment: new (name: string, _?: { defaultValue: int }) => DartInt;
...
@DartClass
@Implements(DartInt)
class JSInt extends JSNumber implements DartInt/*implements int, double*/ {
    constructor(i: int) {
        super(i);
    }
    ...
    get bitLength(): int {
        ...
        if (nonneg >= 0x100000000) {
            ...
            return JSInt._bitCount(JSInt._spread(nonneg)) + 32;
        }
    ...
    static _binaryGcd(x: int, y: int, inv: bool): int {
        ...
        const ac: bool = new DartInt(x).isEven;
        let u = x;
        let v = y;
        let a = 1, b = 0, c = 0, d = 1;
        do {
        ...
        if (!inv) return s * v;
        if (v != 1) throw new DartError("Not coprime");
        if (d < 0) {
            d += x;
            if (d < 0) d += x;
        } else if (d > x) {
    ...
    modInverse(m: int): int {
        ...
        return JSInt._binaryGcd(m, t, true);
    }
    ...
    gcd(other: int): int {
        ...
        if (y == 0) return x;
        if ((x == 1) || (y == 1)) return 1;
        return JSInt._binaryGcd(x, y, false);
    }
    ...
    static _bitCount(i: int): int {
        ...
        i = JSInt._shru(i, 0) - (JSInt._shru(i, 1) & 0x55555555);
        i = (i & 0x33333333) + (JSInt._shru(i, 2) & 0x33333333);
        i = 0x0F0F0F0F & (i + JSInt._shru(i, 4));
        i += JSInt._shru(i, 8);
        i += JSInt._shru(i, 16);
        return (i & 0x0000003F);
    }

    static _shru(value: int, shift: int) {
        return value >>> shift /* JS('int', '# >>> #', value, shift)*/;
    }

    static _shrs(value: int, shift: int) {
        return value >> shift /* JS('int', '# >> #', value, shift)*/;
    }

    static _ors(a: int, b: int) {
 ...
 */
@DartClass
class DartDouble extends DartNumber {
    @defaultFactory
    ...
    static NAN: double = 0.0 / 0.0;
    static INFINITY: double = 1.0 / 0.0;
    static NEGATIVE_INFINITY: double = -DartDouble.INFINITY;
    static MIN_POSITIVE: double = 5e-324;
    static MAX_FINITE: double = 1.7976931348623157e+308;
...
class JSDouble extends JSNumber implements DartDouble /*implements double */ {
    constructor(n: number) {
        super(n);
    }
