...
class _Future<T> extends Promise<T> implements Future<T> {
    ...
    constructor() {
        super((resolve, reject) => {
            ...
            // TODO : is this enough?
            if (resolve) {
                this._addListener(new _FutureListener.whenComplete(null, resolve));
            }
            if (reject) {
                this._addListener(new _FutureListener.catchError(null, reject, null));
            }
        });
    }
...
class DartStreamIterator<T> implements AsyncIterator<T> {
    ...
    async next(value?: any): Promise<IteratorResult<T>> {
        let hasNext = await (this.moveNext() as Promise<bool>);
        return {
            done: !hasNext,
            value: this.current
        };
