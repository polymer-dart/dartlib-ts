...
class _HashMap {
    ...
    get values() {
        return new DartMappedIterable(this.keys, (each) => this[OperatorMethods.INDEX](each));
    }
    containsKey(key) {
...
let DartSetMixin = class DartSetMixin {
    ...
    toList(_) {
        ...
        let i = 0;
        for (let element of this)
            result[OperatorMethods.INDEX_EQ](i++, element);
        return result;
    }
    map(f) {
...
let DartConstantMap = DartConstantMap_1 = class DartConstantMap extends AbstractDartMap {
    ...
    [OperatorMethods.INDEX_EQ](key, val) {
...
let DartListMixin = DartListMixin_1 = class DartListMixin {
    ...
    contains(element) {
        let length = this.length;
        for (let i = 0; i < this.length; i++) {
            if (this[OperatorMethods.INDEX](i) == element)
                return true;
            if (length != this.length) {
                throw new ConcurrentModificationError(this);
            }
    ...
    every(test) {
        let length = this.length;
        for (let i = 0; i < length; i++) {
            if (!test(this[OperatorMethods.INDEX](i)))
                return false;
            if (length != this.length) {
                throw new ConcurrentModificationError(this);
            }
...
let DartMapMixin = class DartMapMixin {
    ...
    containsValue(value) {
        for (let key of this.keys) {
            if (_dart.equals(this[OperatorMethods.INDEX](key), value))
                return true;
        }
        return false;
    }
...
let JSArray = JSArray_1 = class JSArray extends Array {
    ...
    contains(other) {
        for (let i = 0; i < this.length; i++) {
            if (_dart.equals(this[OperatorMethods.INDEX](i), other))
                return true;
        }
        return false;
    }
...
export function diagnoseRangeError(start, end, length) {
    if (isNot(start, 'int')) {
